...
require_once "preferabli-api.php";

class PreferabliForWooCommerceLabelHandler
{
    public function updatePostForLabel($post_object) {
        ...
        if ( has_post_thumbnail ($post_object ) ) {
            if ( $thumb_id = get_post_meta($post_id, "_thumbnail_id", true) ) {
                ...
                if ( array_key_exists("placeholder_ids", $p4wc_options) ) {
                    $placeholder_ids_string = $p4wc_options['placeholder_ids'];
                ...
                } elseif ( !in_array( get_post_type( $thumb_id ), array( 'preferabli-label') ) ) {
                    // it's a valid image that is not from Preferabli...move along.
                    return $post_object;
                }
            }
        }
        ...
        // check timestamps.
        $checked_at_timestamp = get_post_meta( $post_id, '_preferabli_label_checked_at', true );
        ...
        $p4wc_options = get_option( 'p4wc_plugin_options' );
        if ( !$force_update_label && array_key_exists("last_updated_at", $p4wc_options)) {
            $options_last_updated_at = $p4wc_options["last_updated_at"];
        ...
        update_post_meta($post_id, "_preferabli_label_checked_at", $current_timestamp);

        $unique_identifier_method = "product_id";
        $unique_identifier_method_custom_key = null;
        if ( array_key_exists("unique_identifier_method", $p4wc_options) ) {
            $unique_identifier_method = $p4wc_options['unique_identifier_method'];
            if ( array_key_exists("unique_identifier_method", $p4wc_options) ) {
                $unique_identifier_method_custom_key = $p4wc_options['unique_identifier_method_custom_key'];
            }
        }
        if ( !$label_object_id = $this->getLabelObjectIdForProduct($post_object, $thumb_id,$unique_identifier_method, $unique_identifier_method_custom_key) ) {
            if ( !$is_placeholder_image ) { // This should be a Preferabli label.
                delete_post_meta($post_id, "_thumbnail_id");
            }
        } elseif ( $label_object_id <> $thumb_id ) {
    ...
    private function getLabelObjectIdForProduct($post_object, $existing_label_id, $unique_identifier_method, $unique_identifier_method_custom_key = null) {
        ...
        switch ( $unique_identifier_method ) {
            ...
            case ("post_meta"):
                try {
                    if ( !(strlen($unique_identifier_method_custom_key) > 0 ) ) {
                        return false;
                    }
                    $unique_identifier = get_post_meta( $post_object->ID, $unique_identifier_method_custom_key, true );
                    if ( !(strlen($unique_identifier) > 0 ) ) {
                        return false;
                    }
                } catch (Exception $e) {
        ...
        $label_url = PreferabliForWooCommerceApiHandler::getLabelFromPreferabli($unique_identifier);
        ...
        update_post_meta($label_object_id, "_preferabli_label_url", $label_url);
        update_post_meta($label_object_id, "_wp_attached_file", $label_url);
        update_post_meta($label_object_id, "_wp_attachment_metadata", [
            'width'=>600,
            'height'=> 800,
            'file'=> 'preferabli-labels/path/to/file.jpg',
            'sizes'=> ['original' => ['file' => 'preferabli-labels/path/to/file.jpg', 'width'=> 600, 'height'=> 800, 'mime-type'=> 'image/jpeg']]]);
        
        $timestamp = date('Y-m-d H:i:s');
        update_post_meta($label_object_id, "_preferabli_label_expires_at", $timestamp); // Only for debugging.
        update_post_meta($label_object_id, "_preferabli_label_checked_at", $timestamp);
    ...
    private function verifyPostCategoriesAreValidForLabels($post_object) {
        ...
        if  ( array_key_exists("label_whitelisted_category_ids", $p4wc_options) && strlen($p4wc_options["label_whitelisted_category_ids"])>0) {
            $whitelisted_categories_string = $p4wc_options["label_whitelisted_category_ids"];
